...
import main.scala.AddressParsing.{directionalsMap, streetTypeMap}
...
object addressParsingStandalone {
  ...
  def addressCleaning (addressLine1:String,addressLine2:String): String = {



    var addrs1:String = if (addressLine1 != null || addressLine1.length > 0 || addressLine1.equalsIgnoreCase("NULL")) addressLine1 else ""
    val addrs2:String = if (addressLine2 != null || addressLine2.length > 0 || !addressLine1.equalsIgnoreCase("NULL")) addressLine2 else ""
    ...
    if (match2.nonEmpty && addr2.length == 0  )
    {
